...
CzCOVID19Wallet::CzCOVID19Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zCOVID19 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCOVID19Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCOVID19Seed_deprecated()) {
                    LogPrintf("%s: Updated zCOVID19seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zCOVID19 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zCOVID19 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCOVID19Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCOVID19Count(nCountLastUsed);
    else if (!walletdb.ReadZCOVID19Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCOVID19Wallet::Lock()
{
    seedMaster = 0;
}

void CzCOVID19Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCOVID19Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCOVID19(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCOVID19Wallet::LoadMintPoolFromDB()
{
...
void CzCOVID19Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCOVID19Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCOVID19Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zCOVID19Tracker->HasPubcoinHash(pMint.first)) {
...
bool CzCOVID19Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCOVID19(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zCOVID19Tracker which also adds to database
    pwalletMain->zCOVID19Tracker->Add(dMint, true);
...
void CzCOVID19Wallet::SeedToZCOVID19(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCOVID19Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCOVID19Wallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZCOVID19Count(nCountLastUsed);
...
void CzCOVID19Wallet::GenerateDeterministicZCOVID19(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCOVID19Wallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCOVID19Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
