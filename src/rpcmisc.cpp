...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total COVID19 balance of the wallet \n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in COVID19/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in COVID19/kb\n"
    ...
    UniValue zCOVID19Obj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zCOVID19Obj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"COVID19address\"\n"
            "\nReturn information about the given COVID19 address.\n"

            "\nArguments:\n"
            "1. \"COVID19address\"     (string, required) The COVID19 address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"COVID19address\", (string) The COVID19 address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: COVID19 address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are COVID19 addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) COVID19 address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"COVID19address\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"COVID19address\"  (string, required) The COVID19 address to use for the signature.\n"
