...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZCOVID19Stake(spend));
    } else {
        ...
        CCOVID19Stake* COVID19Input = new CCOVID19Stake();
        COVID19Input->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(COVID19Input);
    }
