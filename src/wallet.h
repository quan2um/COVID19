...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT1000IFMN = 3,
    ONLY_NONDENOMINATED_NOT1000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 COVID19at the same time
    ONLY_1000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zCOVID19send
enum ZerocoinSpendStatus {
    ZCOVID19_SPEND_OKAY = 0,                            // No error
    ZCOVID19_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZCOVID19_WALLET_LOCKED = 2,                         // Wallet was locked
    ZCOVID19_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZCOVID19_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZCOVID19_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZCOVID19_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZCOVID19_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZCOVID19_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZCOVID19_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZCOVID19_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZCOVID19_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZCOVID19_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZCOVID19_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZCOVID19_SPENT_USED_ZCOVID19= 14,                      // Coin has already been spend
    ZCOVID19_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZCOVID19_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZCOVID19BackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZCOVID19OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzCOVID19Auto) const;
    ...
    */
    ...
    CzCOVID19Wallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzCOVID19Tracker> zCOVID19Tracker;
    ...
    void setZWallet(CzCOVID19Wallet* zwallet)
    {
        zwalletMain = zwallet;
        zCOVID19Tracker = std::unique_ptr<CzCOVID19Tracker>(new CzCOVID19Tracker(strWalletFile));
    }

    CzCOVID19Wallet* getZWallet() { return zwalletMain; }
    ...
    void setZCOVID19AutoBackups(bool fEnabled)
    {
    ...
    /** zCOVID19reset */
    boost::signals2::signal<void()> NotifyzCOVID19Reset;
