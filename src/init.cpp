...
#include "config/COVID19-config.h"
#endif
...
CzCOVID19Wallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "COVID19.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "COVID19d.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the COVID19 supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 20333, 20333));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in COVID19/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in COVID19/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, COVID19, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in COVID19/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all COVID19 specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-COVID19stake=<n>", strprintf(_("Enable or disable staking functionality for COVID19inputs (0-1, default: %u)"), 1));
//    strUsage += HelpMessageOpt("-zCOVID19stake=<n>", strprintf(_("Enable or disable staking functionality for zCOVID19inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:20333"));
    ...
    strUsage += HelpMessageOpt("-backupzCOVID19=<n>", strprintf(_("Enable automatic wallet backups triggered after each zCOVID19minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zCOVID19backuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zCOVID19backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeCOVID19amount=<n>     " + strprintf(_("Keep N COVID19anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    if (GetBoolArg("-help-debug", false)) {
        strUsage += HelpMessageOpt("-blockversion=<n>", "Override block version to test COVID19 scenarios");
    }
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 20666, 20667));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i-2020 The COVID19 Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that COVID19 is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. COVID19 Core is shutting down."));
...
#endif
    // Make sure only a single COVID19 process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. COVID19 Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //COVID19: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //COVID19 specific: zerocoin and spork DB's
                ...
                // COVID19: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZCOVID19Minted();
                        RecalculateZCOVID19Spent();
                    }
                    RecalculateCOVID19Supply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // COVID19: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of COVID19 Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart COVID19 Core to complete") << "\n";
        ...
        zwalletMain = new CzCOVID19Wallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zCOVID19Wallet
        uiInterface.InitMessage(_("Syncing COVID19wallet..."));

        bool fEnableZCOVID19Backups = GetBoolArg("-backupzCOVID19", true);
        pwalletMain->setZCOVID19AutoBackups(fEnableZCOVID19Backups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zCOVID19Tracker->Init();
...
//    }
//
//    nAnonymizeCOVID19Amount = GetArg("-anonymizeCOVID19amount", 0);
//    if (nAnonymizeCOVID19Amount > 999999) nAnonymizeCOVID19Amount = 999999;
//    if (nAnonymizeCOVID19Amount < 2) nAnonymizeCOVID19Amount = 2;
    ...
    LogPrintf("Anonymize COVID19 Amount %d\n", nAnonymizeCOVID19Amount);
    ...
    /* Denominations
       ...
       For example:
       1COVID19+1000 == (.1COVID19+100)*10
       10COVID19+10000 == (1COVID19+1000)*10
    */
