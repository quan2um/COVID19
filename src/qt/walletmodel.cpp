...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered COVID19 address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 COVID19.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal COVID19:URI (COVID19:XyZ...?message=example)
            {
...
static void NotifyzCOVID19Reset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCOVID19Reset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCOVID19Reset.connect(boost::bind(NotifyzCOVID19Reset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCOVID19Reset.disconnect(boost::bind(NotifyzCOVID19Reset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zCOVID19Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
