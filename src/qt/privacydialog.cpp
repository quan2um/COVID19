...
#include "zCOVID19controldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCOVID19 ought to be enough for anybody." - Bill Gates, 2017
    ui->zCOVID19payAmount->setValidator( new QDoubleValidator(0.0, 14190000.0, 20, this) );
    ...
    ui->labelzCOVID19SyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCOVID19payAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCOVID19_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCOVID19 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zCOVID19, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCOVID19...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCOVID19 in ") +
...
void PrivacyDialog::on_pushButtonSpendzCOVID19_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCOVID19 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zCOVID19, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zCOVID19
        sendzCOVID19();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCOVID19
    sendzCOVID19();
}

void PrivacyDialog::on_pushButtonZCOVID19Control_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZCOVID19ControlDialog* zCOVID19Control = new ZCOVID19ControlDialog(this);
    zCOVID19Control->setModel(walletModel);
    zCOVID19Control->exec();
}

void PrivacyDialog::setZCOVID19ControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCOVID19Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCOVID19()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid COVID19 Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zCOVID19payAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCOVID19 is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " COVID19 </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCOVID19</b>";
    ...
    // use mints from zCOVID19 selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCOVID19ControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZCOVID19ControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zCOVID19 require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zCOVID19"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZCOVID19_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zCOVID19 require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zCOVID19"));
            ui->TEMintStatus->repaint();
            return;
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zCOVID19 transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCOVID19 was spent successfully update the addressbook with the label
    ...
    // Clear zCOVID19 selector in case it was used
    ZCOVID19ControlDialog::setSelectedMints.clear();
    ui->labelzCOVID19Selected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCOVID19 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " COVID19, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCOVID19 Mint");
    ...
    // Clear amount to avoid double spending when accidentally clicking twice
    ui->zCOVID19payAmount->setText ("0");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zCOVID19Tracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCOVID19 </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCOVID19 "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCOVID19 "));
    ui->labelzCOVID19AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCOVID19 </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zCOVID19 </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzCOVID19SyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzCOVID19->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zCOVID19
        ui->pushButtonMintzCOVID19->setEnabled(false);
        ui->pushButtonMintzCOVID19->setToolTip(tr("zCOVID19 is currently disabled due to maintenance."));

        // Spend zCOVID19
        ui->pushButtonSpendzCOVID19->setEnabled(false);
        ui->pushButtonSpendzCOVID19->setToolTip(tr("zCOVID19 is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zCOVID19
        ui->pushButtonMintzCOVID19->setEnabled(true);
        ui->pushButtonMintzCOVID19->setToolTip(tr("PrivacyDialog", "Enter an amount of COVID19 to convert to zCOVID19", 0));

        // Spend zCOVID19
        ui->pushButtonSpendzCOVID19->setEnabled(true);
        ui->pushButtonSpendzCOVID19->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
