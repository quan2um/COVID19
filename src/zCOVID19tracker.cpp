...
CzCOVID19Tracker::CzCOVID19Tracker(std::string strWalletFile)
{
...
CzCOVID19Tracker::~CzCOVID19Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCOVID19Tracker::Init()
{
...
bool CzCOVID19Tracker::Archive(CMintMeta& meta)
{
...
bool CzCOVID19Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCOVID19Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCOVID19Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCOVID19Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCOVID19Tracker::GetSerialHashes()
{
...
CAmount CzCOVID19Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCOVID19Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCOVID19Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCOVID19Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCOVID19Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCOVID19Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCOVID19Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCOVID19Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCOVID19Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCOVID19Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCOVID19Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCOVID19Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCOVID19Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCOVID19Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCOVID19Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCOVID19Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCOVID19Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzCOVID19 from DB\n", __func__, listDeterministicDB.size());
    }
