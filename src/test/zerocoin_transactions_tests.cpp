...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zCOVID19Tracker = unique_ptr<CzCOVID19Tracker>(new CzCOVID19Tracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZCOVID19_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZCOVID19_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
